...
class ImageRequest(BaseModel):
    ...
    filters: Optional[Dict[str, str]] = None  # Optional filters
...
def apply_filters(text: str, filters: Optional[Dict[str, str]]):
    if not filters:
        return text
    filtered_text = text
    for key, value in filters.items():
        if key in filtered_text:
            filtered_text = filtered_text.replace(key, value)
    return filtered_text

# Mise à jour de la fonction gemini_output pour inclure des arguments optionnels
def gemini_output(image_path, system_prompt=None, user_prompt=None, filters=None):
    image_info = image_format(image_path)
    input_prompt = [system_prompt, image_info]
    if user_prompt:
        input_prompt.append(user_prompt)
    response = model.generate_content(input_prompt)
    return apply_filters(response.text, filters)

# Fonction pour générer le contenu avec le modèle
def gemini_output_all(image_path, user_prompt=None, filters=None):
    image_info = image_format(image_path)
...
# Fonction pour traiter le PDF avec filtrage
def process_pdf_content_all(pdf_path, user_prompt=None, filters=None):
    output_folder = "images"
    image_paths = convert_pdf_to_png(pdf_path, output_folder)
    results = []
    for image_path in image_paths:
        output = gemini_output_all(image_path, user_prompt=user_prompt, filters=filters)
...
async def extract_all_words_from_image(request: ImageRequest):
    try:
        ...
        thread = threading.Thread(target=lambda: result.update({"output": gemini_output_all(
            request.image_path, user_prompt=request.user_prompt, filters=request.filters)}))
        thread.start()
        thread.join()
...
async def extract_all_words_from_pdf(request: PDFRequest):
    try:
        ...
        thread = threading.Thread(target=lambda: result.update({"output": process_pdf_content_all(
            request.pdf_path, user_prompt=request.user_prompt, filters=request.filters)}))
        thread.start()
        thread.join()
